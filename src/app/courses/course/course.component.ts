...
export class CourseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    /** @entities$ let us reach all data from store for the specific entity, in this case lessons */
    this.course$ = this.coursesService.entities$.pipe(
      map((courses) => courses.find((course) => course.url == courseUrl))
    );

    /**
     * @withLatestFrom Used to join the course observable wiht the lessons observable
     * it returns a dupla value with an snapshot of lessons observable and the last value of course observable
     */
  ...
  //method to load lessons data from storage
